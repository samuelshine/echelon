#!/usr/bin/env python3
"""Orchestrate the Track B (response-shaped) AI-assisted dual review.

Mirrors scripts/run_ai_review_v03.py exactly, pointed at the response corpus
and scripts/ai_reviewers_response_v03.py's policies. See that module's
docstring for what "AI-assisted" means here and its documented limitations.
"""
...
from scripts.ai_reviewers_response_v03 import expert_adjudicate, reviewer_a_assess, reviewer_b_assess

PROJECT_ROOT = Path(__file__).resolve().parents[1]
QUEUE = PROJECT_ROOT / "data" / "review_v2" / "response_v0_3_review.jsonl"
WORK_ROOT = PROJECT_ROOT / "data" / "review_v2" / "ai_review_response_v03"
SUBMISSIONS = PROJECT_ROOT / "review_submissions" / "response_v0_3"
PUBLIC_MANIFEST = PROJECT_ROOT / "data" / "manifests" / "response_v03_distributed_review_manifest.json"

REVIEWER_A = "ai_reviewer_a"
REVIEWER_B = "ai_reviewer_b"
EXPERT = "ai_claude_expert"
VALID_LABELS = {"malicious_code", "benign"}
...
def run_primary(reviewer_id: str, assess_fn, manifest: dict) -> dict:
    ...
    write_json(SUBMISSIONS / f"{reviewer_id}.json", submission)
    return submission
...
def run_expert(manifest: dict, first: dict, second: dict) -> dict:
    ...
    import_reviews(db_path, reviews)
    submission = export_submission(db_path, kit)
    write_json(SUBMISSIONS / f"{EXPERT}.json", submission)
    return submission
...
def main() -> int:
    ...
    manifest = build_public_manifest(QUEUE, "response_v0_3_review")
    ...
    summary = {
        ...
        "agreement_rate": round(1 - len(conflicts) / len(a_by_id), 4),
