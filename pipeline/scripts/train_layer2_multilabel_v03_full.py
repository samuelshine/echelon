...
distilbert/v03-full-candidate), never overwriting the currently-served `best/`
...
MANIFEST = PROJECT_ROOT / "data" / "manifests" / "layer2_training_manifest_v03_full.json"
SPLIT_ROOT = PROJECT_ROOT / "data" / "splits_v2_v03_full"
OUTPUT_DIR = PROJECT_ROOT / "models" / "layer2-threat-distilbert" / "v03-full-candidate"
METRICS_PATH = PROJECT_ROOT / "models" / "layer2-threat-distilbert" / "metrics_v03_full.json"
...
def metrics_at(probs, labels, source_ids, families):
    ...
    # Track B: response-shaped malicious_code performance, reported as its own
    # slice per LAYER2_RETRAIN_PLAN.md §6 -- never merged into the prompt-shaped
    # malicious_code number above, so a regression in one is never masked by
    # improvement in the other.
    response_mask = np.array([s == "echelon_response_v0_3" for s in source_ids])
    if response_mask.any():
        rc = CAT_INDEX["malicious_code"]
        p, y = probs[response_mask, rc], labels[response_mask, rc]
        pred = (p >= 0.5).astype(int)
        tp = int(((pred == 1) & (y == 1)).sum()); fp = int(((pred == 1) & (y == 0)).sum())
        fn = int(((pred == 0) & (y == 1)).sum())
        prec = tp / (tp + fp) if tp + fp else 0.0
        rec = tp / (tp + fn) if tp + fn else 0.0
        f1 = 2 * prec * rec / (prec + rec) if prec + rec else 0.0
        out["response_shaped_malicious_code"] = {
            "rows": int(response_mask.sum()), "support": int(y.sum()),
            "precision": round(prec, 4), "recall": round(rec, 4), "f1": round(f1, 4),
        }
    else:
        out["response_shaped_malicious_code"] = None
    return out
...
def main() -> int:
    ...
    report = {
        ...
        "provenance_note": (
            ...
            "adjudication, v0.3 Track A's full-coverage AI dual review "
            "(scripts/ai_reviewers_v03.py), and v0.3 Track B's response-shaped AI dual review "
            "(scripts/ai_reviewers_response_v03.py) -- see docs/RESPONSE_CURATION_SPEC.md"
        ),
    }
