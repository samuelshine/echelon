#!/usr/bin/env python3
"""Train + calibrate the multi-label Layer 2 threat classifier on reviewed splits.

Fail-closed: refuses to run unless the training manifest passes the gate. Reads the
leakage-safe JSONL splits, trains a DistilBERT multi-label head over the five Echelon
threat categories (benign = all-zero), calibrates per-category temperatures on
validation, and reports honest per-category + slice metrics on the frozen test split.

Self-contained PyTorch loop (no HF Trainer) for transformers-version stability.
"""
...
MANIFEST = PROJECT_ROOT / "data" / "manifests" / "layer2_training_manifest.json"
SPLIT_ROOT = PROJECT_ROOT / "data" / "splits_v2"
OUTPUT_DIR = PROJECT_ROOT / "models" / "layer2-threat-distilbert" / "best"
METRICS_PATH = PROJECT_ROOT / "models" / "layer2-threat-distilbert" / "metrics.json"
...
def fit_temperatures(logits, labels):
    """Per-category temperature minimizing BCE on validation (grid search)."""
...
def metrics_at(probs, labels, source_ids, families):
    ...
    # Aggregate risk = max calibrated category prob. Benign = all-zero true row.
    ...
    # Defensive-cyber slice (the dedicated false-positive control).
    defensive = np.array([
        s == "echelon_targeted_v0_2" and str(f).startswith("defensive_cyber")
...
def main() -> int:
    validate_training_manifest(MANIFEST)  # fail-closed
    ...
    validate_split_rows(train_rows + val_rows + test_rows)  # combined leakage/eligibility check
    ...
    # Class imbalance -> positive weights per category.
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"epoch {epoch} done in {time.time()-t0:.0f}s | val macro_f1={vm['macro_f1']} temps={np.round(temps,2).tolist()}")
    ...
    # Frozen test evaluation with the calibration chosen on validation.
    ...
    report = {
        ...
        "test": test_metrics,
        "provenance_note": "labels include AI-assisted provisional adjudication of 152 v0.2 conflicts",
    }
